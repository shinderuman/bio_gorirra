...
`
	userPrompt = `新しい「バイオゴリラ」の投稿を1つだけ生成してください。
...
func generateBioGorillaPostWithRetry(ctx context.Context, cfg *Config) (string, error) {
	...
	for i := range maxRetries {
		content, err := generateBioGorillaPost(ctx, cfg)
		if err == nil {
...
func generateBioGorillaPost(ctx context.Context, cfg *Config) (string, error) {
	switch cfg.LLMProvider {
	case "gemini":
		return generateWithGemini(ctx, cfg)
	case "anthropic":
		return generateWithAnthropic(ctx, cfg)
	default:
		return "", fmt.Errorf("unsupported provider: %s", cfg.LLMProvider)
	}
}

func generateWithAnthropic(ctx context.Context, cfg *Config) (string, error) {
...
func generateWithGemini(ctx context.Context, cfg *Config) (string, error) {
